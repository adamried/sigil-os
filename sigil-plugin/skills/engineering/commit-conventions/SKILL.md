...
chainable: false
invokes: []
invoked_by: [developer, orchestrator, security]
...
| `sigil` | Sigil OS workflow artifacts (spec, plan, tasks). Auto-generated by the orchestrator at phase commits. |
| `verified` | **Full-pipeline only, feature-level.** Marks the security pass for a feature. References `.sigil/specs/<feature>/security.md`. Quick Flow does not emit this type. |
...
## The `verified:` Commit Type

Used only at the end of the full-pipeline workflow, after security review passes. It marks a feature as security-verified by referencing the per-feature security report.

**Format:**

```
verified: <feature-slug> security pass

References: .sigil/specs/<feature-dir>/security.md

Refs: <ticket-key>   (if a ticket is in chain context)
```

**Rules:**

- Emitted **once per feature**, not per task — this is a feature-level commit, not a task-level commit.
- Emitted **only by the full-pipeline workflow**. Quick Flow does not run security review and never produces a `verified:` commit.
- The referenced `security.md` file must exist at `.sigil/specs/<feature-dir>/security.md` and contain the security agent's per-feature report. If the file is missing, do not emit the commit — surface the gap to the user instead.
- The orchestrator is responsible for invoking this skill with `action: format` and `type: verified` after security passes and before the push/PR checkpoint.

## Version History

| Version | Date | Changes |
|---------|------|---------|
| 1.1.0 | 2026-05-27 | Add `sigil:` type for orchestrator phase commits. Add `verified:` type for full-pipeline post-security commits (S4-001 FR-A03/A04). |
