...
| Term | What It Means |
|------|---------------|
| **Personal Config** | Your personal settings. Sigil reads from three layers in order: built-in defaults → `~/.sigil/config.yaml` (your global preferences across all projects) → `.sigil/config.yaml` (this project's overrides). Project wins over global. Both YAML files are gitignored. |
| **Global Config** | `~/.sigil/config.yaml` — settings that apply to every Sigil project on your machine. Useful when you always want technical track or audit mode regardless of project. Edit with `/sigil:config set --global <key> <value>`. |
| **Project Config** | `.sigil/config.yaml` — settings just for this project. Wins over global. Edit with `/sigil:config set <key> <value>`. |
...
| **Execution Mode** | A setting that controls how Sigil runs the workflow: `automatic` (Sigil chooses, asks at standard checkpoints), `directed` (you choose specialists and approaches — requires technical track), or `autonomous` (runs end-to-end without per-step prompts; safety gates still pause; ends with a cumulative diff review — requires technical track). |
| **Autonomous Mode** | The execution mode where Sigil runs the entire pipeline without per-step prompts. Safety gates still pause: constitution violations, security blockers, code review "Request changes," QA fix-loop exhaustion, out-of-scope file detection, and fatal errors. Ends with a mandatory cumulative-diff review of the full feature branch. |
| **Safety Gates** | The set of conditions that pause autonomous mode regardless of mode: constitution violations, security blockers (Critical/High), code review blockers, QA fix-loop exhaustion, override expirations, out-of-scope file detection, and fatal errors. |
| **Verified Commit** | A feature-level git commit produced by the full pipeline after security review passes. Format: `verified: <feature> security pass`. References the per-feature security report at `.sigil/specs/<feature>/security.md`. Quick Flow doesn't emit verified commits. |
| **Per-Task Commit** | A git commit produced after each task completes QA validation. Uses Conventional Commits format and references any ticket key from the branch. Out-of-scope file changes are surfaced for explicit include/stash/discard decision before commit. |
| **Feature Branch** | The git branch Sigil creates at the start of implementation, before any task commits. Named per constitution Article 2 if defined, else `sigil/<spec-dir-name>` (e.g., `sigil/003-user-auth`). Local until the push/PR checkpoint at the end. |
| **Feature-Level Status** | A table in `tasks.md` tracking gates that apply to the whole feature (QA, code review, security, verification commit) separately from per-task status. Quick Flow marks security + verification as `[—] N/A (Quick Flow)`. |
| **Audit Mode** | An optional setting that logs every workflow step. Useful for reviewing what happened after a workflow completes. Enable with `/sigil:config set audit_mode true`. |
| **Audit Log** | Per-session audit files at `.sigil/audit/<timestamp>_<feature-slug>.md`. Earlier single-file logs (`.sigil/audit-log.md`) are auto-migrated to the new directory layout on first audit interaction. View with `/sigil:audit` (subcommands: `latest`, `list`, `trace`, `agent`, `events`, `clear`). |
